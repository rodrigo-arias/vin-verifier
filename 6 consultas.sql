...
-- QUERY A
SELECT MAX(pesoEnvio) AS  'Máximo Peso del Año', MIN(pesoEnvio) AS 'Mínimo Peso del Año', MAX(FORMAT(fchEnvio, 'dd/MM/yy')) AS 'Primera Fecha del Año', MIN(FORMAT(fchEnvio, 'dd/MM/yy')) AS 'Última Fecha del Año'
FROM Envios
WHERE YEAR(fchEnvio) = 2015
UNION
SELECT MAX(pesoEnvio), MIN(pesoEnvio), MAX(FORMAT(fchEnvio, 'dd/MM/yy')), MIN(FORMAT(fchEnvio, 'dd/MM/yy'))
FROM Envios
WHERE YEAR(fchEnvio) = 2016
